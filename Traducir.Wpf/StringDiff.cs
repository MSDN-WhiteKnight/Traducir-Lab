...
namespace Traducir.Wpf
{
    class Program
    {
        public static bool CollectionContainsSubsequence<T>(IList<T> coll, IList<T> subsequence)
        {
            if (coll.Count == 0 || subsequence.Count == 0) return false;
            if (coll.Count < subsequence.Count) return false;
            ...
            while (true)
            {
                if (coll_counter >= coll.Count) break;
                if (subsequence_counter >= subsequence.Count) break;
                ...
                if (coll[coll_counter].Equals(subsequence[subsequence_counter]))
                {
                    ...
                    subsequence_counter++;
                    count_equal++;
                    if (count_equal == subsequence.Count) return true;
                    continue;
                }
                else
                {
        ...
        public static IList<T> GetSubCollection<T>(IList<T> coll, int index, int length)
        {
            if (index < 0) index = 0;
        ...
        public static IList<T> GetCollectionsLongestCommonSubsequence<T>(IList<T> left, IList<T> right)
        {
            if (left.Count == 0 || right.Count == 0) return new T[0];

            if (CollectionContainsSubsequence(right, left)) return left;
            if (CollectionContainsSubsequence(left, right)) return right;

            if (left.Count == 1 || right.Count == 1) return new T[0];
            ...
            else
            {
                ...
                if (lcs1.Count > right.Count) return lcs1;

                IList<T> lcs2 = GetCollectionsLongestCommonSubsequence(
                    GetSubCollection(left, 0, left.Count - 1),
                    right);

                if (lcs1.Count > lcs2.Count) return lcs1;
                else return lcs2;
            }
        }

        public static string[] BreakString(string src, int maxlen)
        {
            ...
            while (true)
            {
                ...
                if (i + maxlen > src.Length) len = src.Length - i;
                else len = maxlen;
        ...
        public static double CalcSimilarity(IList<string> common, IList<string> left, IList<string> right)
        {
        ...
        public static double GetStringSimilarity(string left, string right)
        {
                ...
                && right.Length <= 20)
            {
                ...
                //Console.WriteLine("String: " + new string(res.ToArray()));
            ...
            else
            {
                ...
                //Console.WriteLine("Words: " + String.Join(" ", arr_res));
